...
static void MouseWheelCB(int button, int state, int x, int y)
{
  if (gPersProj.FOV >= 1.0f && gPersProj.FOV <= 45.0f)
  {
    if (state > 0)
      gPersProj.FOV -= 1.0;
    else
  ...
  if (gPersProj.FOV <= 1.0f)
    gPersProj.FOV = 1.0f;
  if (gPersProj.FOV >= 45.0f)
    gPersProj.FOV = 45.0f;
}
...
int main(int argc, char** argv)
{
  ...
  glutCreateWindow("Project 15: Camera Control - Part 2");
